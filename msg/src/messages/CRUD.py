...
def create_new_message(message_data: SendMessageData, from_user_id: str, to_user_id: str, db: Session):
    ...
    if sender.user_type != UserType.ADMIN:
        if sender.user_type == reciever.user_type:
            raise HTTPException(
                status_code=400, detail="This type of conversation is not allowed")
    if sender.user_type == UserType.TAILOR and reciever.user_type != UserType.ADMIN:
        check_message = db.query(Message).filter(
            Message.from_user_id == reciever.user_id, Message.to_user_id == sender.user_id).all()
    ...
    new_message = Message(**data, from_user_key=message_data.message_key,
...
def update_message(update_data: UpdateMessage, from_user_id: str, message_id: str, db: Session):
    ...
    if message:
        ...
        if time_difference > timedelta(minutes=2):
            raise HTTPException(
                status_code=400, detail="Cannot update message after 1 minute of creation.")
...
def get_message_history(from_user_id: str, to_user_id: str, db: Session):
    ...
    message1 = db.query(Message).filter(
        from_user_id == Message.from_user_id, to_user_id == Message.to_user_id).all()
    message2 = db.query(Message).filter(
        to_user_id == Message.from_user_id, from_user_id == Message.to_user_id).all()
