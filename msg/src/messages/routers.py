...
def send_message(req_body: SendMessageData, from_user_id: str, to_user_id: str, db=Depends(get_db)):
    print ("This is the req_body:  ", req_body)
...
def get_user_messages(from_user_id: str, to_user_id: str, db=Depends(get_db)):
    message_history = get_message_history(from_user_id, to_user_id, db)
    if message_history:
        for msg in message_history:
            if msg.to_user_id == from_user_id:
                is_viewed(message.id, from_user_id, db)
...
def mark_as_viewed(to_user_id: str,  message_id: str,  db=Depends(get_db)):
...
def get_user_message_lists(user_id: str, db=Depends(get_db)):
