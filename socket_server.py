    ...
    if 'notification_app' not in connected_users:
        ...
        try:
            await notification_socket.connect(
                f'http://localhost:8000/notifications?token={token}',  # Passing token as a query parameter
    ...
    if 'message_list_app' not in msg_list_connected_users:
        try:
            await message_list_socket.connect(
                f'http://localhost:8000/message_lists?token={token}',  # Passing token as a query parameter
                transports=['websocket']
            )
        except socketio.exceptions.ConnectionError as e:
