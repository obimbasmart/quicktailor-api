...
from src.messages.schemas import (MessageHistoryResponse, SendMessageData,
                                  MessageListResponse,  UpdateMessage)
import os
import requests
...
def send_message(req_body: SendMessageData, from_user_id: str, to_user_id: str,  db=Depends(get_db),
    ...
    if from_user_id != current_user.id:
        raise HTTPException(
            status_code=403, detail="Couldn't validate credentials")
    ...
    response = requests.post(
        f"http://127.0.0.1:8001/chats/{from_user_id}/messages/{to_user_id}", json=send_data, headers=headers)
...
def get_messages(from_user_id: str, to_user_id: str,  db=Depends(get_db),
    ...
    response = requests.get(
        f"http://127.0.0.1:8001/chats/{from_user_id}/messages/{to_user_id}",  headers=headers)
    return response.json()
...
def update_message(req_body: UpdateMessage, from_user_id: str, message_id: str,  db=Depends(get_db),
    ...
    response = requests.put(
        f"http://127.0.0.1:8001/chats/{from_user_id}/messages/{message_id}", json=req_body.dict(), headers=headers)
    return response.json()
...
def get_messages(user_id: str,  db=Depends(get_db),
    ...
    response = requests.get(
        f"http://127.0.0.1:8001/chats/{user_id}/messages",  headers=headers)
    return response.json()
