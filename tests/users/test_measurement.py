...
def test_update_user_measurement_success(access_token_user, measurement_update_fields, reset_db):
    ...
    update_response = client.put("/users/measurement/{}".format(access_token_user['id']),json=json_data, headers=access_token_user['header'])
...
def test_invalid_fields(access_token_user, measurement_update_fields, failure_data, reset_db):
    ...
    response = client.put("/users/measurement/{}".format(access_token_user['id']),json=failure_data, headers=access_token_user['header'])
    assert response.status_code == 422, "Expected 200, got {}".format(update_response.status_code)
...
def test_unauthorized_update(access_token_user, access_token_user_02, measurement_update_fields, reset_db):
    ...
    response = client.put("/users/measurement/{}".format(other_user_id),json=json_data, headers=access_token_user['header'])
    assert response.status_code == 401, "Expected 200, got {}".format(update_response.status_code)
