...
def test_update_user_success(access_token_user, info_update_fields, reset_db):
    ...
    update_response = client.put("/users/{}".format(access_token_user['id']),json=info_update_fields.model_dump(), headers=access_token_user['header'])
...
def test_invalid_fields(access_token_user, failure_data, reset_db):
    ...
    update_response = client.put("/users/{}".format(access_token_user['id']),json=failure_data, headers=access_token_user['header'])
    assert update_response.status_code == 422, "Expected 422, got {}".format(update_response.status_code)
...
def test_unauthorized_update(access_token_user, access_token_user_02, info_update_fields, reset_db):
    ...
    update_response = client.put("/users/{}".format(other_user_id),json=info_update_fields.model_dump(), headers=access_token_user['header'])
    assert update_response.status_code == 401, "Expected 401, got {}".format(update_response.status_code)
...
def test_already_used_email(access_token_user, access_token_user_02, info_update_fields_02, reset_db):
    ...
    update_response = client.put("/users/{}".format(other_user_id),json=info_update_fields_02.model_dump(), headers=access_token_user_02['header'])
