...
def test_get_empty_cart(access_token_user):
    res = client.get(f'/carts/{access_token_user["id"]}',headers=access_token_user['header'])
    assert res.status_code == 200

def test_get_cart_failiure_access_denied(access_token_tailor):
    res = client.get(f'/carts/{access_token_tailor["id"]}',headers=access_token_tailor['header'])
    assert res.status_code == 403 

def test_get_cart_failure_unauthorized(access_token_tailor, access_token_user):
    res = client.get(f'/carts/{access_token_tailor["id"]}',headers=access_token_user['header'])
    assert res.status_code == 401
