...
def test_get_user_favorites_success(access_token_user, reset_db):
    ...
    get_response = client.get(
        "/users/favorites/{}".format(access_token_user['id']), headers=access_token_user['header'])
...
def test_get_user_favorites_unauthorized(access_token_user, access_token_user_02, reset_db):
    ...
    get_response = client.get(
        "/users/favorites/{}".format(access_token_user['id']), headers=access_token_user_02['header'])
    assert get_response.status_code == 401, "Expected 401, got {}".format(
        get_response.status_code)
...
def test_wrong_id_add_favorites(access_token_user, access_token_tailor, product_01,  favorite_update_fields, reset_db):
    ...
    product_id = product_01(access_token_tailor)
    tailor_id = access_token_tailor['id']
    product_data = favorite_update_fields(
        None, access_token_user['id']).model_dump()
    ...
    assert response_tailor.status_code == 404, "Expected 404, got {}".format(
        response_tailor.status_code)
    assert response_product.status_code == 404, "Expected 404, got {}".format(
...
def test_add_favorites_unauthorized(access_token_user, access_token_user_02,  access_token_tailor, product_01,  favorite_update_fields, reset_db):
    ...
    assert response_data.status_code == 401, "Expected 401, got {}".format(
